...
###### 将df_admission_original['科系'] 欄位的 '\n' 分隔符替换为 ';' 分隔符. <注意> split('\n')中的分隔符號'\n'要隨資料而決定. 
df_admission_original['科系'] = df_admission_original['科系'].str.replace('\n', ';')
...
###### 畫圖形(單一學系或學院, 比較圖形), 只適用於 Q1
@st.cache_data(ttl=3600, show_spinner="正在處理資料...")  ## Add the caching decorator
def Draw_1(系_院_校, column_index, split_symbol=';', dropped_string='沒有工讀', sum_choice=1, result_df=pd.DataFrame(), selected_options=[], dataframes=dataframes, combined_df=combined_df, width1=10,heigh1=6,width2=11,heigh2=8,width3=10,heigh3=6,title_fontsize=15,xlabel_fontsize = 14,ylabel_fontsize = 14,legend_fontsize = 14,xticklabel_fontsize = 14, yticklabel_fontsize = 14, annotation_fontsize = 14, bar_width = 0.08, fontsize_adjust=0, item_name='', rank=False, rank_number=5, df_admission=df_admission, df_admission_faculty=df_admission_faculty, df_admission_school=df_admission_original, desired_order=desired_order):
    ##### 使用Streamlit畫單一圖
    if 系_院_校 == '0':
        collections = [df_admission, df_admission_faculty, df_admission_school]
        if rank == True:
...
def Draw_2(column_index, split_symbol=';', dropped_string='沒有工讀', sum_choice=1, dataframes=dataframes, combined_df=combined_df, width1=10,heigh1=6,width2=11,heigh2=8,width3=10,heigh3=6,title_fontsize=15,xlabel_fontsize = 14,ylabel_fontsize = 14,legend_fontsize = 14,xticklabel_fontsize = 14, yticklabel_fontsize = 14, annotation_fontsize = 14, bar_width = 0.08, fontsize_adjust=0, item_name='', rank=False, rank_number=5, df_admission=df_admission, df_admission_restrict=df_admission, selected_options=highlight_schools):
    ...
    collections = [df_admission_restrict, df_admission_noHighlight, df_admission]
    if rank == True:
        dataframes = [Frequency_Distribution(df, column_index, split_symbol, dropped_string, sum_choice).head(rank_number) for df in collections]  ## 'dataframes' list 中的各dataframe已經是按照次數高至低的項目順序排列
    else:
        dataframes = [Frequency_Distribution(df, column_index, split_symbol, dropped_string, sum_choice) for df in collections]  ## 'dataframes' list 中的各dataframe已經是按照次數高至低的項目順序排列
...
###### 各學系填答人數與比例
with st.expander("Q4. 各學系填答人數與比例(複選): 在所選擇群體(系院校)之考生, 其重複報考各學系分佈情形."):
    # df_admission.iloc[:,3] ## 
    column_index = 3
    item_name = "在所選擇的群體中, 各學系報考分佈情形(可重複報考不同學系)"
    column_title.append(df_admission.columns[column_index][0:])
    # set(df_admission_original['科系'])
    rank_number = 5
...
with st.expander("Q6-特定高中. 特定高中參與靜宜大學申請入學主要原因(複選):"):
    ...
    # item_name = "重點高中("+selected_options+") V.S.一般高中 參與靜宜大學申請入學主要原因(複選)"
...
#     # Draw(系_院_校, column_index, ';', '沒有工讀', 1, result_df, selected_options, dataframes, combined_df)
#     # Draw(系_院_校, column_index, split_symbol=';', dropped_string='沒有工讀', sum_choice=1, result_df, selected_options)
#     Draw(系_院_校, column_index, split_symbol='\n', dropped_string='沒有工讀', sum_choice=1, result_df=result_df, selected_options=selected_options, dataframes=dataframes, combined_df=combined_df, width1=10,heigh1=6,width2=11,heigh2=8,width3=10,heigh3=6,title_fontsize=20,xlabel_fontsize = 18,ylabel_fontsize = 18,legend_fontsize = 18,xticklabel_fontsize = 18, yticklabel_fontsize = 16, annotation_fontsize = 18, bar_width = 0.08, fontsize_adjust=1, item_name=item_name, rank=False, rank_number=rank_number, df_admission=df_admission_restrict, df_admission_faculty=df_admission_faculty_restrict)    
#     # Draw_2(column_index, split_symbol=';', dropped_string='沒有工讀', sum_choice=1, dataframes=dataframes, combined_df=combined_df, width1=10,heigh1=6,width2=11,heigh2=8,width3=10,heigh3=6,title_fontsize=15,xlabel_fontsize = 14,ylabel_fontsize = 14,legend_fontsize = 14,xticklabel_fontsize = 14, yticklabel_fontsize = 14, annotation_fontsize = 14, bar_width = 0.08, fontsize_adjust=0, item_name='', rank=True, rank_number=5, df_admission=df_admission, df_admission_restrict=df_admission_restrict)    
...
with st.expander("Q10. 一階篩選公告後是否有收到通過學系之聯絡:"):
    ...
    # Draw(系_院_校, column_index, ';', '沒有工讀', 1, result_df, selected_options, dataframes, combined_df)
    # Draw(系_院_校, column_index, split_symbol=';', dropped_string='沒有工讀', sum_choice=1, result_df, selected_options)
    Draw(系_院_校, column_index, split_symbol='\n', dropped_string='沒有工讀', sum_choice=1, result_df=result_df, selected_options=selected_options, dataframes=dataframes, combined_df=combined_df, width1=10,heigh1=6,width2=11,heigh2=8,width3=10,heigh3=6,title_fontsize=22,xlabel_fontsize = 20,ylabel_fontsize = 20,legend_fontsize = 20,xticklabel_fontsize = 20, yticklabel_fontsize = 20, annotation_fontsize = 20, bar_width = 0.08, fontsize_adjust=1, item_name=item_name, rank=False, rank_number=rank_number, df_admission=df_admission, df_admission_faculty=df_admission_faculty)    
...
#     Draw(院_系, column_index, split_symbol='\n', dropped_string='沒有工讀', sum_choice=1, result_df=result_df, selected_options=selected_options, dataframes=dataframes, combined_df=combined_df, width1=10,heigh1=6,width2=11,heigh2=8,width3=10,heigh3=6,title_fontsize=15,xlabel_fontsize = 14,ylabel_fontsize = 14,legend_fontsize = 14,xticklabel_fontsize = 14, yticklabel_fontsize = 14, annotation_fontsize = 14, bar_width = 0.08, fontsize_adjust=0, item_name=item_name, rank=False, rank_number=rank_number)    
...
with st.expander("Q12. 是否有參加4/20或4/27靜宜大學舉辦之甄試說明會:"):
    ...
    Draw(系_院_校, column_index, split_symbol=';', dropped_string='沒有工讀', sum_choice=1, result_df=result_df, selected_options=selected_options, dataframes=dataframes, combined_df=combined_df, width1=10,heigh1=6,width2=11,heigh2=8,width3=10,heigh3=6,title_fontsize=20,xlabel_fontsize = 18,ylabel_fontsize = 18,legend_fontsize = 18,xticklabel_fontsize = 18, yticklabel_fontsize = 18, annotation_fontsize = 18, bar_width = 0.08, fontsize_adjust=1, item_name=item_name, rank=False, rank_number=rank_number, df_admission=df_admission, df_admission_faculty=df_admission_faculty)    
...
with st.expander("Q16. 對於書審資料的準備，你曾經由哪些管道獲得幫助與指引(複選):"):
    ...
    Draw(系_院_校, column_index, split_symbol='\n', dropped_string='沒有工讀', sum_choice=1, result_df=result_df, selected_options=selected_options, dataframes=dataframes, combined_df=combined_df, width1=10,heigh1=6,width2=11,heigh2=8,width3=10,heigh3=6,title_fontsize=18,xlabel_fontsize = 16,ylabel_fontsize = 16,legend_fontsize = 16,xticklabel_fontsize = 16, yticklabel_fontsize = 16, annotation_fontsize = 16, bar_width = 0.08, fontsize_adjust=1, item_name=item_name, rank=False, rank_number=rank_number, df_admission=df_admission, df_admission_faculty=df_admission_faculty)    
